...
CREATE PROCEDURE dbo.copy_data_to_new_database
	@new_db VARCHAR(MAX),
	@dbs_list VARCHAR(MAX)
AS
BEGIN
	SET NOCOUNT ON;
	...
	--Create new database.
	SET @sql_create_new_db = 'CREATE DATABASE ' + QUOTENAME(@new_db);
	EXEC (@sql_create_new_db);
	...
	*/
	...
	WHILE @@FETCH_STATUS = 0
		BEGIN
			--Create a schema in the target database using the names of the source databases as the schema names.
			SET @sql_create_schema = 'USE ' + QUOTENAME(@new_db) + '; IF NOT EXISTS (SELECT * FROM sys.schemas WHERE name = ' + QUOTENAME(@database_name, '''') + ')' + ' BEGIN EXEC(''CREATE SCHEMA ' + QUOTENAME(@database_name) + ';' + ''') END;';
			EXEC (@sql_create_schema);

			--Declare static cursor to loop through all tables in the current database.
			SET @sql_table_cursor =
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
					--Copy each table from the source database into the target database with the new schema.
					SET @sql_insert = 'SELECT * INTO ' + QUOTENAME(@new_db) + '.' + QUOTENAME(@database_name) + '.' + QUOTENAME(@table_name) + ' FROM ' + QUOTENAME(@database_name) + '.dbo.' + QUOTENAME(@table_name);
					EXEC (@sql_insert);
					SET @msg = 'New table created in database: ' + QUOTENAME(@new_db) + ' Table Name: ' + QUOTENAME(@new_db) + '.' + QUOTENAME(@database_name) + '.' + QUOTENAME(@table_name);
	...
	SET @msg = 'Stored procedure completed.';
	RAISERROR(@msg, 0, 1) WITH NOWAIT;
END;
GO
